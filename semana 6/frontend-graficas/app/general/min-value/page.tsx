...
import { fetchMinValue } from "@/app/service/api";
...
export default function page() {
  ...
  const [chartData, setChartData] = useState<ChartDataType>({
    ...
    datasets: [
      {
        label: "Producto con valor minimo",
        data: [],
      ...
      .then((data) => {
        if (data.length > 0) {
          ...
          setChartData({
            ...
            datasets: [
              {
                label: "Valor minimo de productos por tipos",
                data: data.map((item: any) => item.valorMinimo),
                backgroundColor: "rgba(59, 130, 246, 0.4)",
                borderColor: "rgba(59, 130, 246, 1)",
                borderWidth: 2,
              },
